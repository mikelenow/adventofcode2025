...
def count_timelines(grid):
    ...
    while current_row < height - 1 and timelines:
        ...
        print(f"Row {current_row}: {sum(timelines.values())} total timelines across {len(timelines)} columns")
        print(f"  Columns: {dict(sorted(timelines.items()))}")

        # For each column with timelines
        for col, count in sorted(timelines.items()):
            if 0 <= col < width:
                char = grid[current_row][col]
                print(f"  Col {col} ('{char}'): {count} timelines", end="")

                if char == '.' or char == 'S':
                    # Particle continues downward in all timelines
                    next_timelines[col] = next_timelines.get(col, 0) + count
                    print(f" -> continue down")
                elif char == '^':
                    ...
                    print(f" -> split into {count} left + {count} right")
    ...
    print(f"Final timelines: {dict(sorted(timelines.items()))}")
...
def main():
    ...
    print(f"\nTotal number of timelines: {result}")
    print(f"Expected: 40")
